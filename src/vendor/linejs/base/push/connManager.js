...
// GOMI:
import { SquareService_fetchMyEvents_args as gen_SquareService_fetchMyEvents_args, sync_args as gen_sync_args } from "../thrift/readwrite/struct.js";
...
export class ConnManager {
  ...
  async initializeConn(state = 1, initServices = [
    3,
    6,
    8,
    9,
    ...
    const tosendHeaders = this.client.request.getHeader();
  ...
  async buildAndSendSignOnRequest(conn, serviceType, kwargs = {}) {
    this.log("buildAndSendSignOnRequest", {
      serviceType,
      kwargs
    });
  ...
  async InitAndRead(initServices = [
    3,
    5
  ]) {
