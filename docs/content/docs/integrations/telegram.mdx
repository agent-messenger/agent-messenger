...
> **Tip**: After `npm install -g agent-messenger`, `agent-telegram` is available directly. For one-off execution without global install, use `bunx --package agent-messenger agent-telegram ...`.
...
# Send a message
bunx --package agent-messenger agent-telegram message send <chat-id-or-@username> "Hello from Telegram"
```
...
If the env vars are missing, the command prompts for your own `api_id` and `api_hash` from `https://my.telegram.org/apps`. If `agent-messenger` is installed globally, replace the `bunx --package ...` prefix below with `agent-telegram`.
