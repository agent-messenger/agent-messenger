...
Authenticate by scanning a QR code with your phone. The `onQRUrl` callback receives the URL to display, and `onPincode` receives the PIN the user must confirm in the LINE mobile app.
...
Authenticate with email and password. A PIN confirmation step in the LINE mobile app may be required.
