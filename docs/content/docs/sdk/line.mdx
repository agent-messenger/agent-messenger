...
Authenticate by scanning a QR code with your phone. The `onQRUrl` callback receives the URL to display, and `onPincode` receives a confirmation PIN shown on the phone.

```typescript
const result = await client.loginWithQR({
  device: 'DESKTOPMAC',
  onQRUrl: (url) => console.log(`Scan this QR code: ${url}`),
  onPincode: (pin) => console.log(`Confirm PIN on your phone: ${pin}`),
})
...
Authenticate with email and password. A PIN confirmation step may be required.
...
```typescript
// Close the connection (MUST call when done)
client.close()
```

Once closed, any subsequent method call throws a `LineError` with code `not_connected`. Create a new client if you need to reconnect.
