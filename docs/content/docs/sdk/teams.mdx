...
// With expiry tracking: new TeamsClient(token, '2025-03-23T12:00:00Z')
```
...
Channel IDs use UUID format (`19:xxx@thread.tacv2`), not numeric snowflakes. Always pass the parent `teamId` alongside the `channelId`.
...
const client = new TeamsClient(token, tokenExpiresAt)
