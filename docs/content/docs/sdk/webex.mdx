...
const msg = await client.sendMessage(roomId, '**Bold** and _italic_', { markdown: true })

// Send a direct message by email
const dm = await client.sendDirectMessage('alice@example.com', 'Hey!')
const dm = await client.sendDirectMessage('alice@example.com', '**Important**', { markdown: true })
...
const updated = await client.editMessage(messageId, roomId, '**Updated**', { markdown: true })
// → WebexMessage
...
const people = await client.listPeople({ displayName: 'Alice', max: 10 })
// → WebexPerson[]
```
...
const config = await manager.pollDeviceToken(deviceCode, interval, expiresIn, clientId, clientSecret)
// → WebexConfig
