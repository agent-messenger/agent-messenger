...
- `unauthorized` — Bot token is missing, invalid, or revoked
- `rate_limited` — Discord 429 response; check `Retry-After` header
- `forbidden` — Bot lacks the permission/intent for this action
- `not_found` — Channel, guild, or message does not exist
- `bad_request` — Invalid payload (e.g. message too long)
- `gateway_error` — Real-time Gateway connection failure
