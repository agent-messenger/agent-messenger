...
const filtered = await client.searchTeamChatMessages(channelId, 'deployment', { limit: 10 })
...
const filtered = await client.searchUserChatMessages(channelId, 'billing', { limit: 10 })
// → ChannelSearchResponse
```
...
Manages Channel Talk credentials stored at `~/.config/agent-messenger/channeltalk-credentials.json`. Files are written with `0o600` permissions. The environment variables `E2E_CHANNEL_ACCOUNT_COOKIE` (for account cookie), `E2E_CHANNEL_SESSION_COOKIE` (for session cookie), and `E2E_CHANNEL_WORKSPACE_ID` (for current workspace) take precedence over file-based credentials.
...
import {
  ...
  MessagePersonTypeSchema,
  PersonEntitySchema,
  SortOrderSchema,
  UserChatStateSchema,
  MessageBlockArraySchema,
} from 'agent-messenger/channeltalk'
```
