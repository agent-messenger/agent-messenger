...
try {
  ...
  if (err instanceof ChannelBotError) {
    ...
    if (err.code === 'unauthorized') {
      // Access key/secret is invalid or revoked
    } else if (err.code === 'not_found') {
      // Group, chat, or bot does not exist
    }
  } else {
    throw err
  }
...
- `unauthorized` — Access key or secret is missing, invalid, or revoked
- `forbidden` — Credentials lack permission for this workspace or action
- `not_found` — Workspace, group, chat, manager, or bot does not exist
- `bad_request` — Invalid payload (e.g. malformed message blocks)
- `rate_limited` — Channel Talk API rate limit exceeded
- `network_error` — Connection failure to the Channel Talk API
