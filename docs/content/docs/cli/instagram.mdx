...
- Authenticates with **username and password**, with support for 2FA and login challenges.
...
agent-instagram message search <thread> <query>
agent-instagram message search 12345678901 "meeting notes"
...
The `<thread>` argument accepts a thread ID, which is a numeric string (e.g. `12345678901`).
