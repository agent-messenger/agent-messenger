...
# Schedule a message (post-at is Unix timestamp)
agent-slack message schedule <channel> <text> --post-at <timestamp>
agent-slack message schedule general "Friday update" --post-at 1700000000 --thread <ts>
...
agent-slack message scheduled-delete <channel> <id>

# Send an ephemeral message (visible to one user only)
agent-slack message ephemeral <channel> <user> <text>
...
# Set your status
agent-slack user set-status --text "In a meeting" --emoji :calendar:
```
...
```bash
# Add a reminder (time is Unix timestamp or Slack time string)
agent-slack reminder add <text> <time>
agent-slack reminder add "Review PR" 1700000000
agent-slack reminder add "Team standup" "in 2 hours" --user U0ABC123
