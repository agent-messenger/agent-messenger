...
`auth status` checks stored credential state only. Kakao validates the access token when a command opens a LOCO session. If the token has expired or been revoked, commands fail with structured JSON:
...
Run `agent-kakaotalk auth login` before retrying. Other explicit server rejections use `code: "login_rejected"`; connection closes and timeouts remain `code: "login_failed"` so callers can distinguish authentication from transport failures.
