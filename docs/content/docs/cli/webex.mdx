...
```bash
# 1. Log in (opens browser automatically, zero-config!)
...
### OAuth Device Grant (Zero-Config)

agent-webex uses OAuth Device Grant flow with built-in Integration credentials. No tokens to copy, no developer portal setup needed.
...
| --- | --- | --- |
| OAuth Device Grant (default) | 14-day access, 90-day refresh (auto-refresh) | Interactive use, development |
