---
title: Quick Start
description: Get up and running with Agent Messenger in 30 seconds.
icon: Rocket
---

## Installation

```bash
npm install -g agent-messenger
```

Or use your favorite package manager:

```bash
# Using pnpm
pnpm add -g agent-messenger

# Using yarn
yarn global add agent-messenger

# Using bun
bun add -g agent-messenger
```

This installs eight CLI tools:

- `agent-slack` — Slack CLI (user token, zero-config)
- `agent-slackbot` — Slack Bot CLI (bot token, for server-side/CI/CD)
- `agent-discord` — Discord CLI (user token, zero-config)
- `agent-discordbot` — Discord Bot CLI (bot token, for server-side/CI/CD)
- `agent-teams` — Microsoft Teams CLI
- `agent-telegram` — Telegram CLI (user account via TDLib)
- `agent-channeltalk` — Channel Talk CLI (beta, zero-config, extracted cookies)
- `agent-channeltalkbot` — Channel Talk Bot CLI (beta, API credentials, for server-side/CI/CD)

## Extract Credentials

Agent Messenger automatically extracts tokens from your desktop apps. Make sure the app is installed and you're logged in.

```bash
# For Slack
agent-slack auth extract

# For Discord
agent-discord auth extract

# For Teams
agent-teams auth extract

# For Telegram — interactive login with auto API credential provisioning
agent-telegram auth login

# For Channel Talk (beta) — auto-extracted from desktop app
agent-channeltalk auth extract
```

That's it. No OAuth flows. No API tokens. No configuration files.

## Verify Setup

Check that authentication worked:

```bash
agent-slack auth status
```

You should see your workspace name and user info.

## Send Your First Message

```bash
# Get a snapshot of your workspace
agent-slack snapshot --pretty

# Send a message to a channel
agent-slack message send general "Hello from the CLI!"
```

## Output Formats

By default, all commands output compact JSON (perfect for AI agents):

```bash
agent-slack channel list
# {"channels":[{"id":"C01234567","name":"general",...}]}
```

Add `--pretty` for human-readable output:

```bash
agent-slack channel list --pretty
```

## Multi-Workspace Support

If you're logged into multiple workspaces:

```bash
# List all workspaces
agent-slack workspace list

# Switch to a different workspace
agent-slack workspace switch <workspace-id>

# Check current workspace
agent-slack workspace current
```

## Next Steps

- [Slack Reference](/docs/slack) - Full command reference
- [Slack Bot Reference](/docs/slackbot) - Bot token for server-side/CI/CD
- [Discord Reference](/docs/discord) - Full command reference
- [Discord Bot Reference](/docs/discordbot) - Bot token for server-side/CI/CD
- [Teams Reference](/docs/teams) - Full command reference
- [Telegram Reference](/docs/telegram) - TDLib setup and command reference
- [Channel Talk Reference](/docs/channeltalk) - Full command reference (beta)
- [Channel Talk Bot Reference](/docs/channeltalkbot) - Bot API for Channel Talk (beta)
- [Agent Skills](/docs/agent-skills) - Integrate with AI coding agents
